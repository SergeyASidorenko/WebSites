...
window.addEventListener('load', function(e) {
    ...
    dateInput.addEventListener('blur', function() {
        var xhttp = new XMLHttpRequest();
        var el = this;
        xhttp.open("GET", "order.php?date=" + dateInput.value, true);
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                if (this.responseText == 'format') {
                    el.style.backgroundColor = "tomato";
    ...
    timeInput.addEventListener('blur', function() {
        var xhttp = new XMLHttpRequest();
        var el = this;
        xhttp.open("GET", "order.php?date=" + dateInput.value + "&time=" + el.value, true);
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                if (this.responseText == 'format') {
                    el.style.backgroundColor = "tomato";
                    var errEl = el.parentNode.getElementsByClassName('error')[0];
                    errEl.innerHTML = 'Неверный формат';
                } else if (this.responseText == 'hours') {
                    el.style.backgroundColor = "tomato";
                    var errEl = el.parentNode.getElementsByClassName('error')[0];
                    errEl.innerHTML = 'Не ранее 10 часов';
                }
            }
        };
    ...
    function addClickEventToOkButton(el) {
        el.addEventListener('click', function(e) {
            ...
            if (existingGoodIndex != -1 && amount.value.trim() == '') {
                goods.splice(existingGoodIndex, 1);

            } else if (existingGoodIndex != -1) {
